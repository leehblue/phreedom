...
class Test_Storm extends R66_Test {
  ...
  public function test_find_multiple_objects_with_one_condition_ordered() {
    $success = false;
    $error = null;
    $storm = new R66_Storm('my_test_table');
    $result = $storm->find_where('last_name', '=', 'Blue', 'first_name');
    
    if(!is_array($result)) {
      $success = false;
      $error = "Result of find_where should be an array of storms";
    }
    elseif(count($result) !=2 ) {
      $success = false;
      $error = "Expecting 2 objects to be found but instead found: " . count($result);
    }
    elseif($result[0]->first_name == 'Emily' || $result[1]->first_name == 'Lee') {
      $success = true;
    }
    else {
      $success = false;
      $error = "Result set in unexpected order: " . $result[0]->first_name . ', ' . $result[1]->first_name;
    }
    $this->check($success, $error);
  }
  ...
  public function test_copy_from_should_populate_storm() {
    $data = array(
      'id' => 12,
      'first_name' => 'Test',
      'last_name' => 'Person'
    ...
    $storm = new R66_Storm('my_test_table');
    $storm->copy_from($data);
    $internal = $storm->get_data();
    $this->check($data == $internal, "Expected: " . print_r($data, true) . " :: Found: " . print_r($internal, true));
  }
  
  public function test_attribute_unique_should_return_true_if_attribute_is_unique() {
    $data = array(
      'id' => 12,
      'first_name' => 'Test',
    ...
    $storm = new R66_Storm('my_test_table');
    $storm->copy_from($data);
    $this->check($storm->attribute_unique('last_name'), "The last name attribute should be unique but it was not: " . $storm->last_name);
  }
  
  public function test_attribute_unique_should_return_false_if_attribute_is_not_unique() {
    $data = array(
      'id' => 2,
      'first_name' => 'Lee',
      'last_name' => 'Blue'
  ...
  public function test_attribute_unique_should_return_true_if_the_value_is_only_in_its_own_row() {
    $data = array(
      'first_name' => 'Test',
      'last_name' => 'Unique'
    );
    ...
    $storm->copy_from($data);
    $storm->save();
    $this->check($storm->attribute_unique('last_name'), "The last name attribute should be unique but it was not: " . $storm->id . ': ' . $storm->last_name);
  }
